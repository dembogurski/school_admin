...
while( $Q0->NextRecord() ){
    ...
    if( ($el['FAM']!=$old['FAM'])||($el['GRUPO']!=$old['GRUPO'])||($el['TEMPORADA']!=$old['TEMPORADA']) ){
        if($sup['detallado'] === 'Detallado'){
        ...
        $subtotal0_SUBTOTAL = 0; 
			   ...
			   if($el['FAM']!=$old['FAM']){
					 if ($flag > 1) {   echo "<tr> <td> &nbsp; </td> <td colspan='4'><b>Sector ".$old['FAM']."  ( $FAM ) Subtotal: (".number_format($FAM_S,2,',','.').") </b></td>  </tr>"; } 
					  $FAM = 0 +$el['CANT'];
					  $FAM_S  = 0 +$el['SUBTOTAL'];
			   }else{ 
			       	  $FAM += 0 + $el['CANT'];   
