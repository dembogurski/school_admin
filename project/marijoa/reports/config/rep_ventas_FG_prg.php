...
//$query0 = "SELECT distinct  p.p_fam as FAM, p.p_grupo as GRUPO,p.p_tipo as TIPO,  d.df_cantidad as CANT, d.df_subtotal as SUBTOTAL  FROM factura f,det_factura d, mnt_prod p WHERE f.fact_nro = d.df_fact_num and d.df_cod_prod = p.p_cod and f.fact_localidad LIKE '%' and f.fact_cli_ci like '%' and p.p_fam like '%' and p.p_grupo like  '%' and p.p_tipo like '%'  and  f.fact_fecha between '2009-01-01' and '2009-01-03'  and f. fact_estado = "Cerrada" and f.fact_cli_cat like '%' and fact_moneda like  '%' order by p.p_fam , p.p_grupo , p.p_tipo,d.df_cantidad asc    ";
...
while( $Q0->NextRecord() ){
    ...
    if( ($el['FAM']!=$old['FAM'])||($el['GRUPO']!=$old['GRUPO']) ){
        ...
        $subtotal0_SUBTOTAL = 0; 
 		       ...
 		       if( trim($el['GRUPO'])!=trim($old['GRUPO'])){ 
				  if ($flag > 1) {  echo "<tr> <td> ".$old['FAM']." </td> <td colspan='1' align='left' > ".$old['GRUPO']."    </td>   <td align='right'> ". number_format( $GRUPO ,2,'.',',')."  </td>  <td align='right' > ".number_format($GRUPO_S,2,',','.')." </td> </tr>";} 
				  $GRUPO = 0 + $el['CANT'];
