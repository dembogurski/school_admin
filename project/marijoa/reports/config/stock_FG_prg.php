...
//$query0 = "SELECT  p.p_fam as FAM, p.p_grupo as GRUPO,  p.p_cant as CANT, p.p_cant * p.p_compra as VALOR_AL_COSTO  FROM  mnt_prod p WHERE  p.p_local LIKE  '04' and p.p_fam like '%' and p.p_grupo like  '%' and p.p_cant > 0  and p.p_cod like '%' order by p.p_fam , p.p_grupo ,p.p_cant desc";
...
while( $Q0->NextRecord() ){
    ...
    if( (trim($el['FAM'])!=trim($old['FAM']))||(trim($el['GRUPO'])!=trim($old['GRUPO'])) ){
