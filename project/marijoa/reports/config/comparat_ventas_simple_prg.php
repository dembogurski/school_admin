...
$j = 0;
  ...
  $T->Set('suc',"Todas");   
	  ...
	  $sql = "SELECT p.p_fam as FAM,p.p_grupo AS GRUPO, p.p_tipo AS TIPO, p.p_color AS COLOR,  sum(d.df_cantidad)  AS CANT_V ,   (d.df_cantidad - p.p_cant)  as DIFF  ,f.fact_localidad AS SUC
	  ...
	  d.df_cod_prod = p.p_cod AND  f.fact_localidad like '%' AND  f.fact_fecha BETWEEN '$desde' AND '$hasta' and fact_estado ='Cerrada' group by p.p_fam,p.p_grupo,p.p_tipo,p.p_color  order by CANT_V desc, p.p_fam,p.p_grupo,p.p_tipo,p.p_color asc";	
...
while( $Q0->NextRecord() ){
    ...
    if( ($el['FAM']!=$old['FAM'] )||($el['GRUPO']!=$old['GRUPO'])||($el['TIPO']!=$old['TIPO']||($el['COLOR']!=$old['COLOR']) ) ){
    		...
    		$Q1->Query("SELECT  sum(p.p_cant)  AS CANT_STOCK FROM   mnt_prod p 
	  		WHERE p.p_fam like '$_OLD_FAM' AND p.p_grupo like '$_OLD_GRUPO' AND p.p_tipo like '$_OLD_TIPO' AND p.p_clasif like '$CLASIF' AND p.p_estruc like '$ESTRUC' and p.p_color like '$_ODL_COLOR'   AND 	p.p_local like '%'  AND p.p_cant > 0  AND prod_fin_pieza <> 'Si' ;");
    		$Q1->NextRecord();
		    ...
		    if ($DIFF < 0) {
                         $DIFF_1 = $DIFF * - 1; 
		    	$T->Set('query0_DIFF','<td align="right" > <input style="text-align: right;color:black;font-weight:bolder; background-color:orange;" type="text" name="'.$color_id4.'"   class="diff" size="8" value="'.number_format($DIFF_1,2,',','') .'" readonly="readonly"/> </td>' );   
		    }else {
                       
		    	$T->Set('query0_DIFF','<td style="text-align: right;"> <input type="text" name="'.$color_id4.'"  class="diff" size="8" value="'.number_format(0,2,',','') .'" readonly="readonly"/> </td>' );  
		    }  
