...
//$query0 = "SELECT distinct  p.p_GRUPO as GRUPO, p.p_COLOR as COLOR,p.p_tipo as TIPO,  d.df_cantidad as CANT, d.df_subtotal as SUBTOTAL  FROM factura f,det_factura d, mnt_prod p WHERE f.fact_nro = d.df_fact_num and d.df_cod_prod = p.p_cod and f.fact_localidad LIKE '%' and f.fact_cli_ci like '%' and p.p_GRUPO like '%' and p.p_COLOR like  '%' and p.p_tipo like '%'  and  f.fact_fecha between '2009-01-01' and '2009-01-03'  and f. fact_estado = "Cerrada" and f.fact_cli_cat like '%' and fact_moneda like  '%' order by p.p_GRUPO , p.p_COLOR , p.p_tipo,d.df_cantidad asc    ";
...
while( $Q0->NextRecord() ){
    ...
    if( ($el['GRUPO']!=$old['GRUPO'])||($el['COLOR']!=$old['COLOR']) ){
        if($sup['detallado'] === 'Detallado'){
        ...
        $subtotal0_SUBTOTAL = 0; 
 		       ...
 		       if( trim($el['COLOR'])!=trim($old['COLOR'])){
				  if ($flag > 1) {  echo "<tr> <td>&nbsp;&nbsp; ".$old['GRUPO']." </td> <td colspan='1' align='left' >&nbsp;&nbsp; ".$old['COLOR']."    </td>   <td align='right'> ". number_format( $COLOR ,2,'.',',')."  &nbsp;&nbsp;</td>  <td align='right' > ".number_format($COLOR_S,2,',','.')." &nbsp;&nbsp;</td> </tr>";}
				  $COLOR = 0 + $el['CANT'];
			   ...
			   if($el['GRUPO']!=$old['GRUPO']){
					 if ($flag > 1) {   echo "<tr> <td colspan='2'>&nbsp;&nbsp;<b>".$old['GRUPO']."   </b></td> <td align='right'><b> ". number_format( $GRUPO,2,'.',',')."  &nbsp;&nbsp;</b></td>  <td align='right'> <b> ".number_format($GRUPO_S,2,',','.')."</b> &nbsp;&nbsp;</td>  </tr>"; }
					  $GRUPO = 0 +$el['CANT'];
