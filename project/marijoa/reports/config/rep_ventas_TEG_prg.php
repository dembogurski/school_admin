...
if($sup['detallado'] === 'Detallado'){
	$T->Set('TEMPORADA','Detallado');
}else{
	$T->Set('TEMPORADA','Resumido');
}
...
while( $Q0->NextRecord() ){
    ...
    if( ($el['ESTRUCTURA']!=$old['ESTRUCTURA'])||($el['GRUPO']!=$old['GRUPO'])||($el['TEMPORADA']!=$old['TEMPORADA']) ){
        if($sup['detallado'] === 'Detallado'){
        ...
        $subtotal0_SUBTOTAL = 0; 
				 ...
				 if ($flag > 1) {  echo "<tr>   <td colspan='5' align='left' ><b>TEMPORADA ".$old['TEMPORADA']." ( $TEMPORADA )  Subtotal: (".number_format($TEMPORADA,2,',','.').") </b> </td>  </tr>";} 
				  $TEMPORADA = 0 + $el['CANT'];
			   ...
			   if($el['ESTRUCTURA']!=$old['ESTRUCTURA']){
					 if ($flag > 1) {   echo "<tr> <td> &nbsp; </td> <td colspan='4'><b>ESTRUCTURA ".$old['ESTRUCTURA']."  ( $ESTRUCTURA ) Subtotal: (".number_format($ESTRUCTURA_S,2,',','.').") </b></td>  </tr>"; } 
