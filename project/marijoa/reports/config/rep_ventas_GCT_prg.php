...
//$query0 = "SELECT distinct  p.p_CLASIFICACION as CLASIFICACION, p.p_grupo as GRUPO,p.p_tipo as TIPO,  d.df_cantidad as CANT, d.df_subtotal as SUBTOTAL  FROM factura f,det_factura d, mnt_prod p WHERE f.fact_nro = d.df_fact_num and d.df_cod_prod = p.p_cod and f.fact_localidad LIKE '%' and f.fact_cli_ci like '%' and p.p_CLASIFICACION like '%' and p.p_grupo like  '%' and p.p_tipo like '%'  and  f.fact_fecha between '2009-01-01' and '2009-01-03'  and f. fact_estado = "Cerrada" and f.fact_cli_cat like '%' and fact_moneda like  '%' order by p.p_CLASIFICACION , p.p_grupo , p.p_tipo,d.df_cantidad asc    ";
...
while( $Q0->NextRecord() ){
    ...
    if( ($el['CLASIFICACION']!=$old['CLASIFICACION'])||($el['GRUPO']!=$old['GRUPO'])||($el['TIPO']!=$old['TIPO']) ){
        if($sup['detallado'] === 'Detallado'){
        ...
        $subtotal0_SUBTOTAL = 0; 
		      ...
		      if($el['CLASIFICACION']!=$old['CLASIFICACION']){
					 if ($flag > 1) {   echo "<tr> <td> &nbsp; </td>  <td colspan='3'><b>CLASIFICACION ".$old['CLASIFICACION']."  ( $CLASIFICACION ) Subtotal: (".number_format($CLASIFICACION_S,2,',','.').") </b></td>  </tr>"; } 
