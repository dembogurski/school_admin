...
$query = "SELECT  p.p_fam AS FAM ,  sum(d.df_cantidad) AS CANT,sum(d.df_subtotal) AS SUBTOTAL,sum( (d.df_subtotal) - (((p.p_compra * -1 ) - (p.p_compra * p_porc_recargo / 100)) * -1 ) * d.df_cantidad) AS MARGEN 
...
'$suc' AND p.p_fam LIKE '$fam'  AND  f.fact_fecha BETWEEN '$desde2' AND '$hasta2'  
AND f. fact_estado = 'Cerrada'   GROUP BY p.p_fam   ORDER BY SUBTOTAL DESC ,MARGEN DESC   ";
...
while( $Q0->NextRecord() ){
    ...
    if($array == null){
        $array = array($el['FAM'],0,0,0,$el['CANT'],$el['SUBTOTAL'],$el['MARGEN']);
        $master[$cod_fam]=$array; 
    }else{
      $array[4]= $el['CANT'];
      $array[5]= $el['SUBTOTAL'];
