...
while( $Q0->NextRecord() ){
    ...
    if( ($el['COLOR']!=$old['COLOR'])||($el['GRUPO']!=$old['GRUPO'])||($el['TEMPORADA']!=$old['TEMPORADA']) ){
        if($sup['detallado'] === 'Detallado'){
        ...
        $subtotal0_SUBTOTAL = 0; 
			   ...
			   if($el['COLOR']!=$old['COLOR']){
					 if ($flag > 1) {   echo "<tr> <td> &nbsp; </td> <td colspan='4'><b>COLOR ".$old['COLOR']."  ( $COLOR ) Subtotal: (".number_format($COLOR_S,2,',','.').") </b></td>  </tr>"; } 
					  $COLOR = 0 +$el['CANT'];
