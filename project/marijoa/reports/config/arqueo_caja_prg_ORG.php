...
while( $Q0->NextRecord() ){
    ...
    // VENTAS AL CONTADO
    $db->Query("SELECT SUM(f.fact_total) AS CONTADO FROM factura f WHERE f.fact_vend_cod LIKE '$vendedor' AND f.fact_localidad = '$suc'
                AND f.fact_fecha  BETWEEN '$desde' AND '$hasta' AND f.fact_estado =  'Cerrada' AND f.fact_tipo = 'Contado' ");
    $db->NextRecord();
    ...
    $Z_CONTADO += 0 + $contado;
     ...
     // VENTAS A CREDITO
    $db->Query("SELECT SUM(f.fact_total) AS CREDITO FROM factura  f WHERE f.fact_vend_cod LIKE '$vendedor' AND f.fact_localidad = '$suc'
                AND f.fact_fecha  BETWEEN '$desde' AND '$hasta' AND f.fact_estado =  'Cerrada' AND f.fact_tipo = 'Credito' ");
    $db->NextRecord();
    ...
    $db->Query("SELECT SUM(f.fact_total - d.dp_moneda_ref) AS T_CREDITO
    FROM factura f, fact_detalle_pg d  WHERE f.fact_nro = d.dp_fact_nro AND  f.fact_vend_cod LIKE '$vendedor' AND
    f.fact_localidad = '$suc' AND f.fact_fecha BETWEEN '$desde' AND '$hasta' AND f.fact_estado = 'Cerrada' AND d.dp_conv_tipo = 'Tarjeta Credito' ORDER BY fact_vend_cod ASC");
    $db->NextRecord();
    ...
    $db->Query("SELECT SUM(f.fact_total - d.dp_moneda_ref) AS T_DEBITO
    FROM factura f, fact_detalle_pg d  WHERE f.fact_nro = d.dp_fact_nro AND  f.fact_vend_cod LIKE '$vendedor' AND
    f.fact_localidad = '$suc' AND f.fact_fecha BETWEEN '$desde' AND '$hasta' AND f.fact_estado = 'Cerrada' AND d.dp_conv_tipo = 'Tarjeta Debito'  ORDER BY fact_vend_cod ASC");
    $db->NextRecord();
    ...
    $db->Query("SELECT SUM(f.fact_total - d.dp_efectivo) AS CONVENIO
    FROM factura f, fact_detalle_pg d  WHERE f.fact_nro = d.dp_fact_nro AND  f.fact_vend_cod LIKE '$vendedor' AND
    f.fact_localidad = '$suc' AND f.fact_fecha BETWEEN '$desde' AND '$hasta' AND f.fact_estado = 'Cerrada' AND d.dp_conv_tipo = 'Convenio' ORDER BY fact_vend_cod ASC");
    $db->NextRecord();
    ...
    $db->Query("SELECT SUM(d.dp_moneda_ref) AS EFECTIVO
    FROM factura f, fact_detalle_pg d  WHERE f.fact_nro = d.dp_fact_nro AND  f.fact_vend_cod LIKE '$vendedor' AND
    f.fact_localidad = '$suc' AND f.fact_fecha BETWEEN '$desde' AND '$hasta' AND f.fact_estado = 'Cerrada' AND d.dp_conv_tipo like '%' ORDER BY fact_vend_cod ASC");
    $db->NextRecord();
    ...
    //  Cheques de terceros
    $db->Query("SELECT SUM(c.chq_valor) AS CHEQUES
    FROM factura f,  cheq_terceros c  WHERE f.fact_nro = c.chq_ref AND  f.fact_vend_cod LIKE '$vendedor' AND
    f.fact_localidad = '$suc' AND f.fact_fecha BETWEEN '$desde' AND '$hasta' AND f.fact_estado = 'Cerrada' and c.chq_fecha_pag BETWEEN '$desde' AND '$hasta' ORDER BY fact_vend_cod ASC");
    $db->NextRecord();
    $CHEQUES = $db->Record['CHEQUES'];
    $T->Set('CHEQUES', number_format($CHEQUES,0,',','.'));
    $Z_CHQ_AL_DIA +=0+$CHEQUES;

    $db->Query("SELECT SUM(c.chq_valor) AS CHEQUES_DIF
    FROM factura f,  cheq_terceros c  WHERE f.fact_nro = c.chq_ref AND  f.fact_vend_cod LIKE '$vendedor' AND
    f.fact_localidad = '$suc' AND f.fact_fecha BETWEEN '$desde' AND '$hasta' AND f.fact_estado = 'Cerrada' and c.chq_fecha_pag > '$hasta'  ORDER BY fact_vend_cod ASC");
    $db->NextRecord();
    ...
    //  Cuotas
    $db->Query("SELECT SUM(c.ct_total) AS CUOTAS
    FROM factura f,  cuotas c  WHERE f.fact_nro = c.ct_ref AND  f.fact_vend_cod LIKE '$vendedor' AND
    f.fact_localidad = '$suc' AND f.fact_fecha BETWEEN '$desde' AND '$hasta' AND f.fact_estado = 'Cerrada'  ORDER BY fact_vend_cod ASC");
    $db->NextRecord();
    ...
    $db->Query("SELECT SUM(m.cm_entrada) AS CUOTAS_EFECTIVO
    FROM caja_mov m, factura f  WHERE m.cm_compl LIKE 'Cobro Cuota Nro%' AND f.fact_nro = SUBSTRING(m.cm_compl,23,7)   AND  f.fact_vend_cod LIKE '$vendedor' AND
    f.fact_localidad = '$suc' AND m.cm_fecha BETWEEN '$desde' AND '$hasta' AND f.fact_estado = 'Cerrada'  ORDER BY fact_vend_cod ASC");
    $db->NextRecord();
    ...
    $db->Query("SELECT SUM(m.cm_entrada) AS CUOTAS_CONVENIO
    FROM caja_mov m, factura f  WHERE m.cm_compl LIKE 'Cobro Cuota Convenio:%' AND f.fact_nro = SUBSTRING(m.cm_compl,23,7)   AND  f.fact_vend_cod LIKE '$vendedor' AND
    f.fact_localidad = '$suc' AND m.cm_fecha BETWEEN '$desde' AND '$hasta' AND f.fact_estado = 'Cerrada'  ORDER BY fact_vend_cod ASC");
    $db->NextRecord();
...
cm_entrada_ref AS E_REF, cm_salida_ref AS S_REF  FROM caja_mov WHERE (cm_con LIKE '9-%' OR cm_con LIKE '33-1' OR cm_con LIKE '33-2' OR cm_con LIKE '8-1' OR ( cm_con LIKE '8-2' AND cm_moneda != 'G$' ))  AND cm_empr = '$suc' AND cm_fecha between '$desde' and '$hasta' ");
