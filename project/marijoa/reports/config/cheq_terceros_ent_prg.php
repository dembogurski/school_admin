...
$query0 = "SELECT t.id, chq_ref AS REF,__local as SUC,chq_tipo as TIPO,chq_nro_rec AS REC, chq_bco AS BANCO, chq_cta AS CUENTA, chq_num AS NRO,date_format(chq_fecha_pag,'%d-%m-%Y') AS FECHA_PAG,"
        ...
        . "AND ($sql_recibido) AND ($sql_recibido2) and chq_tipo like '$chq_tipo' order by chq_moneda asc, $order_by $orderc  ";
...
while( $Q0->NextRecord() ){
    ...
    if($old['MON']!= $el['MON'] && $old['MON']!= ''){
        $T->Set('moneda', str_replace("$","s",$old['MON']));
    ...
    $T->Set('query0_VALOR_NO_FORMAT', $el['VALOR'] );
    $T->Set('query0_MON', $el['MON']);
    $T->Set('mon', str_replace("$","s",$el['MON']));
...
$endConsult = true;
if( $endConsult ){
    $T->Set('moneda', str_replace("$","s",$el['MON']));
    $T->Show('query0_total_row');
}
