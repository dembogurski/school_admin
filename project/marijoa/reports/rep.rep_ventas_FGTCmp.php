...
$Obj->query = "'SELECT  p.p_fam AS FAM , p.p_grupo as GRUPO,p.p_tipo as TIPO, sum(d.df_cantidad) AS CANT,sum(d.df_subtotal) AS SUBTOTAL,sum( (d.df_subtotal) - (((p.p_compra * -1 ) - (p.p_compra * p_porc_recargo / 100)) * -1 ) * d.df_cantidad) AS MARGEN   FROM factura f,det_factura d, mnt_prod p WHERE f.fact_nro = d.df_fact_num AND d.df_cod_prod = p.p_cod AND f.fact_localidad LIKE '+el['rep_localidad']+' AND p.p_fam LIKE '+el['p_fam']+' and p.p_grupo LIKE '+el['p_grupo']+' and p.p_tipo LIKE '+el['p_tipo']+'  AND  f.fact_fecha BETWEEN '+el['desdeinv']+' AND '+el['hastainv']+'  AND f. fact_estado = |{Cerrada}|   GROUP BY p.p_fam, p_grupo, p_tipo  ORDER BY SUBTOTAL DESC ,MARGEN DESC   '";
$Obj->del_prg = "";
$Obj->del_tpl = "";
$Obj->tot = "1";
$Obj->pre_sub = "1";
