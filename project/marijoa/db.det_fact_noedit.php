<?php
$Obj->name = "det_fact_noedit";
$Obj->alias = "Detalle Factura Visualizar";
$Obj->help = "Detalle Factura Visualizar";
$Obj->copy_from = "";
$Obj->Inheritance = "";
...
$Obj->Add(
    array(
        ...
        F_TYPE_ => "text",
        F_MAKE_QUERY_ => "1",
        F_QUERY_ => "'select p.p_valmin, p.p_grupo,p.p_tipo, p.p_color,p.p_fam, p.p_descri, p.p_estruc, p.p_lisoest, p.prod_fin_pieza from mnt_grupo g, mnt_clasif cl ,mnt_prod p where  p.p_cod = '+df_cod_prod.getVal()+' and p.prod_fin_pieza = |{No}| '",
...
$Obj->Add(
    array(
        ...
        C_SHOW_ => "db('p_tipo')!=''",
        C_CHANGE_ => "false",
        F_FORMULA_ => "if( df_concept_vta.get()=='Venta'  ){  db('p_descri')+'-'+db('p_tipo')+'-'+db('p_lisoest')+'-'+db('p_color')+'-'+db('p_grupo') } else if( df_concept_vta.get()=='Devolucion' ){'Devolucion'}  else  {'Descuento'} ",
        G_SHOW_ => "238",
        G_CHANGE_ => "238"));
...
$Obj->Add(
    array(
        ...
        F_NODB_ => "1",
        F_ORD_ => "54",
        C_SHOW_ => "df_precio.getVal()<df_p_valmin.getVal()",
...
$Obj->Add(
    array(
        ...
        F_ORD_ => "62",
        F_INLINE_ => "1",
        C_SHOW_ => "sup['fact_estado']=='Empaque'",
        G_SHOW_ => "238",
        G_CHANGE_ => "238"));
...
$Obj->Add(
    array(
        ...
        F_ORD_ => "63",
        F_INLINE_ => "1",
        C_SHOW_ => "sup['fact_estado']=='Empaque'&&df_cant_corte.get()!='0'",
        C_VIEW_ => "false",
...
$Obj->Add(
    array(
        ...
        C_VIEW_ => "false",
        C_CHANGE_ => "false",
        F_FORMULA_ => "if(df_cantidad.getVal() > df_cant_corte.getVal() ){  'Aumentar'   }else { 'Disminuir'  }",
        G_SHOW_ => "238",
        G_CHANGE_ => "238"));
...
$Obj->Add(
    array(
        ...
        F_HELP_ => "Ajustar",
        F_TYPE_ => "proc",
        F_QUERY_ => "'select ajust_stock_desconta('+df_vendedor.getStr()+', '+df_cod_prod.getVal()+','+ df_fact_num.getVal()+','+df_cant.getVal()+','+df_oper.getStr()+','+__user.getStr()+','+df_monto_desc.getVal()+')'",
        F_NODB_ => "1",
        F_ORD_ => "66",
        F_INLINE_ => "1",
        C_SHOW_ => "sup['fact_estado']=='Empaque'&&df_cant_corte.getVal()>0&&(df_cant.getVal()>0  )",
        G_SHOW_ => "238",
        G_CHANGE_ => "238"));
...
$Obj->Add(
    array(
        ...
        F_NODB_ => "1",
        F_ORD_ => "85",
        C_SHOW_ => "df_precio.getVal()<df_p_valmin.getVal()||df_subtotal.getVal()<=0 ",
        C_VIEW_ => "false",
        C_CHANGE_ => " ",
...
$Obj->Add(
    array(
        ...
        F_NODB_ => "1",
        F_ORD_ => "145",
        C_SHOW_ => "df_verif.getVal()>0&&df_confirmar.get()!=''||df_precio.getVal()<df_p_valmin.getVal()||df_subtotal.getVal()<=0 ",
        C_VIEW_ => "false",
        F_FORMULA_ => "disableAcceptButton()",
        G_SHOW_ => "238",
        G_CHANGE_ => "238"));
...
$Obj->Add(
    array(
        ...
        F_NODB_ => "1",
        F_ORD_ => "155",
        C_SHOW_ => "df_fin_piez.get()=='Si'||((df_disponible.getVal()-df_cantidad.getVal())<-5 )",
        C_VIEW_ => "false",
        F_FORMULA_ => "disableAcceptButton()",
        G_SHOW_ => "238",
        G_CHANGE_ => "238"));
