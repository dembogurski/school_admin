<?php
$Obj->name = "venta_confec";
$Obj->alias = "Venta por Confeccion";
$Obj->help = "Factura de Venta de Productos x Confeccion";
$Obj->copy_from = "";
$Obj->Inheritance = "";
$Obj->File = "factura";
$Obj->Filter = "";
...
$Obj->Add(
    array(
        ...
        F_OPTIONS_ => "DB",
        F_MAKE_QUERY_ => "1",
        F_QUERY_ => "'SELECT venta_gen('+fact_iva.getStr()+','+ fact_emp_dir.getStr()+','+fact_emp_tel.getStr()+','+ fact_cli_ci.getStr()+','+ fact_cli_cat.get()+','+fact_nom_cli.getStr()+','+ fact_tipo.getStr()+','+ fact_fecha.getStr()+', '+fact_ruc.getStr()+' ,'+ fact_vend_cod.getStr()+' , '+func_nombre.getStr()+','+fact_localidad.getStr()+','+fact_nro_orden.getStr()+',100);'",
        F_QUERY_REF_ => "fact_nro.firstSQL&&operation==CONSULT_&&fact_fecha.get()!=''&&func_nombre.get()!=''&&fact_cli_ci.get()!='1'&&cambiar_cli.get()&&(fact_cli_cat.getVal()>=fact_r0.getVal()&& fact_cli_cat.getVal() <=  fact_r1.getVal())",
        F_LENGTH_ => "12",
        F_DEC_ => "0",
        F_BROW_ => "1",
        F_REQUIRED_ => "1",
...
$Obj->Add(
    array(
        ...
        F_NODB_ => "1",
        F_ORD_ => "121",
        C_SHOW_ => "fact_nro.getVal()!=''&&func_nombre.get()!='__NO DATA__'&&fact_sin_cod.get()=='xxxxx' && fact_aceptar.get()==''",
        G_SHOW_ => "238",
        G_CHANGE_ => "238"));
...
$Obj->Add(
    array(
        F_NAME_ => "fact_confec",
        F_ALIAS_ => "Detalle de Confecciones",
        F_HELP_ => "Detalle de Confecciones",
        F_TYPE_ => "subform",
        F_OPTIONS_ => "'c_ref='+fact_nro.get()",
        F_LINK_ => "db.inv_confec",
        F_SEND_ => "fact_nro.get()",
        F_NODB_ => "1",
        F_ORD_ => "121",
        C_SHOW_ => "fact_nro.getVal()!=''&&func_nombre.get()!='__NO DATA__'&&fact_sin_cod.get()=='Confeccion' && fact_aceptar.get()==''",
        G_SHOW_ => "238",
        G_CHANGE_ => "238"));		
...
$Obj->Add(
    array(
        F_NAME_ => "ins_lote",
        F_ALIAS_ => "Insertar Lote",
        F_HELP_ => "Insertar Lote",
        F_TYPE_ => "select_list",
        F_OPTIONS_ => "No,Si",
        F_NODB_ => "1",
        F_ORD_ => "156",
        C_SHOW_ => "(fact_nro.getVal()!=''&&func_nombre.get()!='__NO DATA__'&&fact_sin_cod.get()=='Confeccion' && fact_aceptar.get()=='')||operation==CHANGE_",
        G_SHOW_ => "238",
        G_CHANGE_ => "238"));		
...
$Obj->Add(
    array(
        F_NAME_ => "codigos",
        F_ALIAS_ => "Codigo,mts tela,medida;",
        F_HELP_ => "Formato: Codigo,mets tela a usar,medida del producto; usar punto (.) como separador decimal",
        F_TYPE_ => "text",
        F_MULTI_ => "1.20",
        F_LENGTH_ => "5000",
		C_SHOW_ => "fact_ver_caja.get()!='__NO DATA__'&&fact_gen_venta.get()=='Si'&&fact_aceptar.get()!='OK'||fact_nro.get()>0 && fact_estado.get()=='Abierta' && ins_lote.get()=='Si'",
        F_NODB_ => "1",
        F_ORD_ => "157",
        G_SHOW_ => "64",
        G_CHANGE_ => "64"));
...
$Obj->Add(
    array(
        ...
        F_TYPE_ => "text",
        F_MAKE_QUERY_ => "1",
        F_QUERY_ => "'SELECT actualizar_factura('+fact_nro.getVal()+' ,'+ fact_vend_cod.getStr() +',0);' ",
        F_QUERY_REF_ => "fact_aceptar.firstSQL&&fact_finalizar.get()=='Si'",
        F_LENGTH_ => "12",
        F_NODB_ => "1",
        F_ORD_ => "161",
...
$Obj->Add(
    array(
        F_NAME_ => "tipo_conf",
        F_ALIAS_ => "Tipo de Confeccion",
        F_HELP_ => "Tipo de Confeccion",
        F_TYPE_ => "select_list",
        F_OPTIONS_ => "Mantel,Matelasse,Edredon,Fundas,Sabanas,Bata,Rieles",
        F_NODB_ => "1",
        F_ORD_ => "180",     
